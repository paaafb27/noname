...
class QuasarzoneScraper:
    ...
    def _extract_item(self, row):
        ...
        # 판매처
        store_str = title_element.select_one()
        ...
        # 배송비
        delivery_fee_selector = "//div[@class='fm_best_widget _bd_pc']//li[contains(@class, 'li_best2_hotdeal0')]//span[contains(text(), '배송')]/a[@class='strong']"
        delivery_fee_element = row.locator(f"xpath={delivery_fee_selector}")
        delivery_fee = delivery_fee_element.get_text(strip=True) if price_element else ''
